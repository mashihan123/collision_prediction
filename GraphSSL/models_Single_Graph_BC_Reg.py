...
class GraphClassificationModel(nn.Module):
	...
	def __init__(self, feat_dim, hidden_dim, n_layers, gnn, output_dim, load=None):
	...
	def forward(self, data):
		...
		# edge_score = self.edge_classifier(edge_feat)
		regression_score = self.regression(embeddings[0])
		scores = self.classifier(embeddings[0])
		return scores, regression_score
